...
describe('Forgot Password', () => {
  ...
  it('should send an email with a reset password link', async () => {
    const { userRepository, userService, emailService, tokenService, user } = generateUserMock()

    vi.spyOn(tokenService, 'sign').mockReturnValue('mocked-jwt-token')
    vi.spyOn(userRepository, 'getUser').mockResolvedValueOnce(user)
    vi.spyOn(emailService, 'sendResetPasswordEmail').mockResolvedValueOnce(undefined)
    ...
    expect(userRepository.getUser).toHaveResolvedWith(user)
  ...
  it('should return success with passed email when user does not exist', async () => {
    const { userRepository, userService, emailService, tokenService } = generateUserMock()
    ...
    expect(userRepository.getUser).toHaveBeenCalledWith({ email: requestEmail, provider: 'API' })
